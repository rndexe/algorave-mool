...
void main() {
    ...
    // Dimensions of each "shooting star" streak (like a narrow rectangle)
    vec2 boxHalfSize = vec2(0.0, 0.2); // x = width/2 (0), y = height/2
    ...
    for (float starIndex = 0.9; starIndex < 20.0; starIndex += 1.0) {

        // Compute rotation matrix that varies per star
        float angle = starIndex;
        rotationMatrix = mat2(
            cos(angle), -sin(angle),
            sin(angle),  cos(angle)
        );
        ...
        vec2 movingUV = uv * starIndex * 0.1 + iTime * boxHalfSize;
        ...
        vec2 clamped = clamp(position, -boxHalfSize, boxHalfSize);
        ...
        vec4 color = (cos(position.y / 0.1 + vec4(0.0, 1.0, 2.0, 3.0)) + 1.0);
