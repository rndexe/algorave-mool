...
void main() {
  ...
  vec2 offset = (fragCoord + fragCoord - resolution); 
  vec2 uv = offset / resolution.y;

   // Convert Cartesian coordinates to polar-like space
  float logRadius = log(length(uv));        
  // radial component, scaled logarithmically
  float angle = atan(uv.x, uv.y);
  // angular component (atan(x, y))
  ...
  float distortion = 
      length(cos(polarCoords.x) - sin(polarCoords));
  ...
  vec4 finalColor = 
      baseColor / (distortion * radialDistance);
